...
function MatchCard({ match }) {
  return (
    <div className="bg-white/2 border border-white/5 rounded-3xl p-6 hover:border-white/10 transition-all group flex flex-col gap-4">
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <Link
              to={`/user/${match._id}`}
              className="font-black text-base group-hover:text-[#4F86C6] transition-colors"
            >
              {match.name}
            </Link>
            <p className="text-[10px] text-gray-600">{match.email}</p>
            {match.reputationScore > 0 && (
              <p className="text-[10px] text-[#FF7849] font-black mt-0.5">
                ★ {match.reputationScore} ({match.totalReviews} reviews)
              </p>
            )}
          </div>
        </div>
      ...
      {match.bio && (
        <p className="text-xs text-gray-500 italic border-l border-white/10 pl-3 line-clamp-2">
          "{match.bio}"
        </p>
      )}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <p className="text-[8px] text-[#FF7849] uppercase tracking-widest font-black mb-1.5">
            Teaches
          </p>
          <div className="flex flex-wrap gap-1.5">
            {match.teachingSkills?.slice(0, 3).map((s, i) => (
              <span
                key={i}
                className="text-[10px] bg-[#FF7849]/10 border border-[#FF7849]/20 text-[#FF7849] px-2 py-0.5 rounded-full font-black"
              >
                {s.skillName}
              </span>
            ))}
        ...
        <div>
          <p className="text-[8px] text-[#4F86C6] uppercase tracking-widest font-black mb-1.5">
            Wants to Learn
          </p>
          <div className="flex flex-wrap gap-1.5">
            {match.learningSkills?.slice(0, 3).map((s, i) => (
              <span
                key={i}
                className="text-[10px] bg-[#4F86C6]/10 border border-[#4F86C6]/20 text-[#4F86C6] px-2 py-0.5 rounded-full font-black"
              >
                {s.skillName}
              </span>
            ))}
          ...
          <div className="max-w-7xl mx-auto space-y-10">
            <div className="flex flex-col gap-8">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.4em] text-[#4F86C6] font-black mb-1">
                    Smart Matching
                  </p>
                  <h2 className="text-4xl font-black tracking-tight uppercase">
                    Discover <span className="text-[#4F86C6]">Mentors</span>
                  </h2>
                </div>
                <Link
                  to="/dashboard"
              ...
              <form
                ...
                className="relative group w-full lg:max-w-3xl flex gap-3"
              >
                ...
                <button
                  type="submit"
                  className="px-6 py-3 bg-[#4F86C6] text-black font-black text-[10px] uppercase tracking-widest rounded-[24px] hover:bg-[#6a9fd4] transition-all flex-shrink-0"
                >
                  Search
                </button>
              </form>
            ...
            {loading ? (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                  <div
                    key={i}
                    className="h-64 bg-white/3 rounded-3xl animate-pulse"
                  />
                ))}
              </div>
            ) : matches.length === 0 ? (
              <div className="col-span-full py-20 text-center border border-dashed border-white/5 rounded-[40px]">
                ...
                <p className="text-gray-600 text-xs mt-2">
                  Try adding more skills to your profile or broaden your search.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            ...
            {totalPages > 1 && (
              <div className="flex justify-center gap-3">
                <button
                  ...
                  className="text-[10px] font-black px-4 py-2 border border-white/10 rounded-xl hover:border-white/30 disabled:opacity-30 transition-all"
                >
                  ← Prev
                </button>
