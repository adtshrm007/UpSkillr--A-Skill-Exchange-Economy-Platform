...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      toast({ message: `Welcome back, ${res.data.user.name}!`, type: "success" });
      navigate("/dashboard");
    } catch (err) {
    ...
    <div className="flex flex-col lg:flex-row min-h-screen w-full overflow-x-hidden bg-[#0A0A0A] text-white font-mono selection:bg-[#4F86C6]/30">
      ...
      <div className="w-full lg:w-1/2 bg-[#121212] p-8 lg:p-20 flex flex-col justify-between border-r border-white/5 relative overflow-hidden">
        ...
        <div className="relative z-10 mt-12 lg:mt-0">
          <p className="text-[#4F86C6] text-xs font-bold tracking-[0.4em] uppercase mb-4">Verification Required</p>
          <h1 className="text-6xl md:text-8xl xl:text-9xl font-black leading-tight tracking-tighter">
            WELCOME <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#4F86C6] to-white">BACK.</span>
          </h1>
          <p className="text-gray-500 text-lg mt-6 max-w-sm leading-relaxed">
            Re-enter the ecosystem to continue your{" "}
            <span className="text-[#FF7849] font-bold italic underline decoration-1">growth sprint</span>.
          </p>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center items-center p-8 lg:p-24 relative">
        <form onSubmit={handleLogin} className="w-full max-w-md space-y-10 animate-in fade-in slide-in-from-bottom duration-700">
          <header className="space-y-2">
            <h2 className="text-3xl font-black tracking-tight uppercase italic underline decoration-[#FF7849] decoration-4">Log In</h2>
            <p className="text-gray-500 text-sm font-medium">Identify yourself to access your dashboard.</p>
          </header>
          ...
          <div className="grid grid-cols-2 gap-4">
            <button type="button" className="flex items-center justify-center gap-3 bg-white/[0.03] border border-white/10 py-3 rounded-xl hover:bg-white/[0.08] hover:border-white/20 transition-all group">
              <img src={google} alt="google" className="w-5 h-5 grayscale group-hover:grayscale-0 transition-all" />
              <span className="text-[10px] font-bold uppercase text-gray-400">Google</span>
            </button>
            <button type="button" className="flex items-center justify-center gap-3 bg-white/[0.03] border border-white/10 py-3 rounded-xl hover:bg-white/[0.08] hover:border-white/20 transition-all group">
              <img src={instagram} alt="instagram" className="w-5 h-5 grayscale group-hover:grayscale-0 transition-all" />
          ...
          <p className="text-gray-500 text-xs text-center font-medium">
            New to the network?{" "}
            <Link to="/signup" className="text-[#FF7849] font-black uppercase tracking-tighter hover:underline">
              Create Account
            </Link>
          </p>
        </form>
